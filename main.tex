...
present each of these projects, and try to shed some light on both the positive
...
When we talk about simulators in the following, we refer to simulation engines
with support for physically simulating both robot kinematics and dynamics as well
as physical objects in the environment, and where the control and sensor interface
is close to or the same as it would be on real robots. For this paper in particular,
we further only consider engines which can render and animate a human agent
suitable for HRI, and which have been applied for this use case (many engines
could theoretically support this use case, but few of them have actually have been).
For other robot simulation work, a good overview of recent research is~\cite{Ando2010}.

Popular engines with explicit support for controlling a human agent
include USARSim (Lewis~\etal\cite{lewis2007usarsim}), which is very common
for Rescue Robotics, and used in dozens of HRI studies, 
MORSE (Echeverria~\etal\cite{morse_simpar_2012}), obviously, and there have been
also been experiments with character animation in the widely-used simulator
Gazebo (Koenig and Howard~\cite{Koenig2004}).
...
All of these simulators are based around game engines (the Unreal Tournament Development Kit (UDK)~
\footnote{\url{http://www.unrealengine.com/udk/} (UDK)}, the Blender Game 
Engine\footnote{\url{http://wiki.blender.org/index.php/Doc:2.6/Manual/Game_Engine}},
and Ogre3D\footnote{\url{http://www.ogre3d.org/}}, respectively).
Their support for human agents varies drastically, with UDK and Blender offering
much higher-level support, particularly for facial animation, than Ogre3D. However,
all of these require the developer to specify the agents' behavior in detail.
In practice, this limits human agent behaviors to relatively simple motions
and interactions. Hence, it is not surprising that almost all HRI simulation so
far is carried out in \emph{tele-operation} settings, where only the robot
and the environment, but not the human agent, need to be simulated.
...
That said, the use cases shown in the following will demonstrate that
with even current functionality many relevant scenarios can already be realized.
...
Before discussing the use cases, we'd like to make a short remark on the 
\emph{cost} of integrating MORSE into these scenarios. For that, have 
tried to differentiate how much code is needed for integrating MORSE with existing
tools. 

In general, all of the use cases discussed could be \emph{integrated} in 
between 100 to 300 lines of code (specifically: 263LoC for~\ref{sc:assessment},
179 for~\ref{sc:expectations}, 115 for~\ref{sc:navigation}, 
136 for~\ref{sc:generation}, and 229 for~\ref{sc:ci}). This does not cover all 
the scenario code, just the Morse specific parts, but it includes not just Python
code but also shell scripts and launch files for running the scenarios in Morse.

Of course these numbers are hard to interpret in isolation, but they
indicate--in our opinion--that integration with MORSE is relatively little
effort.

\subsection{Which Place for HRI Simulation?}
...
if it was to be conducted on real robots.  Also mentioned, the simulator plays
an important role in several international projects, where it is often the only
way to share a common experimental setting.
...
interaction was not yet immersive (no head tracking, limited, non-dynamic field of
